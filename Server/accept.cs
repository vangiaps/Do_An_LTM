...
namespace Server
{
    public partial class accept : UserControl
    {
        ...
        {
            // 1. Xóa sạch ListBox hiện tại để tránh trùng lặp
            lstPendingUsers.Items.Clear();

            // 2. Lấy danh sách TẤT CẢ các username
            //    đang chờ trong dictionary (an toàn cho luồng)
            var waitingUsernames = ServerNetwork.PendingClients.Keys;

            // 3. Thêm tất cả vào ListBox
            foreach (string username in waitingUsernames)
            {
                lstPendingUsers.Items.Add(username);
            }
        }
